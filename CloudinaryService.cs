...
{
    public static async Task<string?> UploadDocument(this CloudinaryService service, string filePath, string fileName)
    {
        try
        {
            ...
            return await service.UploadDocumentAsync(fileBytes, fileName);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"❌ Error en UploadDocument: {ex.Message}");
            return null;
        }
    }
}
